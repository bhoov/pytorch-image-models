"""Personal submission kit for submitting sbatch jobs with pytorch lightning"""
import argparse
import os
...
def create_lightning_job(
    ...
    print("\n\t\tMYLOGDIR: ", logdir)
    if logdir is None:
        base_logdir = Path.home() / f"scratch/tb_logs"
        # logdir = Path.home() / f"scratch/tb_logs/{job_name}"
    else:
        # logdir = Path(logdir) / job_name
        base_logdir = Path(logdir)

    logdir = base_logdir / job_name
    stdlogs_dir = logdir / "stdlogs"
    stdlogs_dir.mkdir(exist_ok=True, parents=True)

    py_cmd = cmd + f" --devices $GPUS_PER_NODE --num_nodes $WORLD_SIZE --exp_dir {base_logdir} --exp_name {job_name}"
